...
void Ghost::Collisions(Map map) {
    ...
    // if the ghost is not in the center of the path, it cannot turn onto that path
...
void Ghost::CalculateNextDirection() { 
	if (current_direction == Direction::none || ((current_direction == Direction::right || current_direction == Direction::left) && (possible_directions[Direction::up] || possible_directions[Direction::down])) || ((current_direction == Direction::up || current_direction == Direction::down) && (possible_directions[Direction::right] || possible_directions[Direction::left]))) {
		int dir = ofRandom(0, 4);
		while (!possible_directions[dir] || (dir == current_direction) || (current_direction == Direction::right && dir == Direction::left) || (current_direction == Direction::left && dir == Direction::right) || (current_direction == Direction::up && dir == Direction::down) || (current_direction == Direction::down && dir == Direction::up)) {
			dir = ofRandom(0, 4);
		}
